...
**The fallback source of truth for colors, typography, and tokens.** Every diagram draws from semantic roles — not from arbitrary hex values. In Open Design, prefer the active project design system first; use this file only when the project has no clearer brand direction.

Default skin is a cool editorial palette — white-smoke paper, jet-black ink, atomic-tangerine accent, blue-slate muted. It is designed to look good out of the box, but do not mutate this plugin file during a run. Map the active design system or user-supplied brand notes onto these semantic roles in the generated HTML.
...
## Applying a project skin

Use this mapping when the active design system provides stronger direction:

1. Map background/page colors to `paper` and `paper-2`.
2. Map primary text or stroke to `ink`.
3. Map secondary text and quiet lines to `muted`, `soft`, and `rule`.
4. Pick exactly one brand accent for `accent`; derive `accent-tint` from it.
5. Map brand display/body/mono fonts onto `title`, `node-name`, and `sublabel`.
