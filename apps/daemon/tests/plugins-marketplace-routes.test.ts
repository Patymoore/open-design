...
describe('marketplace routes workspace access', () => {
  it('requires workspace manager access for marketplace mutations', async () => {
    const dataDir = process.env.OD_DATA_DIR;
    if (!dataDir) throw new Error('OD_DATA_DIR is required for daemon route tests');

    const listResp = await jsonFetch<{ marketplaces: Array<{ id: string; trust: string }> }>(`${baseUrl}/api/marketplaces`);
    expect(listResp.status).toBe(200);
    const targetMarketplaceId = listResp.body.marketplaces[0]?.id;
    expect(targetMarketplaceId).toBeTruthy();
    ...
    const workspaceResp = await jsonFetch<{ workspace: { id: string } }>(`${baseUrl}/api/workspaces`, {
      ...
      body: JSON.stringify({ name: `Marketplace Access ${Date.now()}` }),
    });
    expect(workspaceResp.status).toBe(200);

    const memberUserId = `marketplace-member-${Date.now()}`;
    ...
    try {
      await assertForbiddenAdmin(await jsonFetch(`${baseUrl}/api/marketplaces`, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({
      ...
      await assertForbiddenAdmin(await jsonFetch(`${baseUrl}/api/marketplaces/${encodeURIComponent(targetMarketplaceId!)}`, {
        method: 'DELETE',
      }));

      await assertForbiddenAdmin(await jsonFetch(`${baseUrl}/api/marketplaces/${encodeURIComponent(targetMarketplaceId!)}/refresh`, {
        method: 'POST',
      }));

      await assertForbiddenAdmin(await jsonFetch(`${baseUrl}/api/marketplaces/${encodeURIComponent(targetMarketplaceId!)}/trust`, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ trust: 'trusted' }),
      }));
    } finally {
    ...
    const afterListResp = await jsonFetch<{ marketplaces: Array<{ id: string }> }>(`${baseUrl}/api/marketplaces`);
    expect(afterListResp.status).toBe(200);
    expect(afterListResp.body.marketplaces.some((marketplace) => marketplace.id === targetMarketplaceId)).toBe(true);
  });
});
