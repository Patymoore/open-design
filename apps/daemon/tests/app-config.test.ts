...
describe('app-config workspace access', () => {
  ...
  it('requires workspace manager access for app config and Orbit routes', async () => {
    ...
    try {
      await assertForbiddenAdmin(await fetch(`${started.url}/api/app-config`));
      await assertForbiddenAdmin(await fetch(`${started.url}/api/app-config`, {
        method: 'PUT',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ agentCliEnv: { OPENAI_API_KEY: 'member-secret' } }),
      }));
      await assertForbiddenAdmin(await fetch(`${started.url}/api/orbit/status`));
      await assertForbiddenAdmin(await fetch(`${started.url}/api/orbit/run`, { method: 'POST' }));
    } finally {
